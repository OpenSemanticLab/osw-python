...
class Quantities():
    ...
    def dbpediaToWikidata(self):
        ...
        for dbp in self.dbpdict:
            w = ""
            for uri in self.dbpdict[dbp].get('http://www.w3.org/2002/07/owl#sameAs', []):
                if "wikidata.org/entity/" in uri: 
                    w = uri
                    break
            self.dbpwdict[dbp] = w
        
        print("length self.dbpwdict: ", len(self.dbpwdict)) # § create json instead of print

        # create dict qudt -> wikidata
        self.qwdict = {}
        for w in self.wdict:
            if 'http://www.wikidata.org/prop/direct-normalized/P8393' in self.wdict[w]: self.qwdict[self.wdict[w]['http://www.wikidata.org/prop/direct-normalized/P8393'][0]] = w
        print(len(self.qwdict))
        self.qdbpdict = {}
        for q in self.qdict:
            if 'http://qudt.org/schema/qudt/dbpediaMatch' in self.qdict[q]:
                dbp = self.qdict[q]['http://qudt.org/schema/qudt/dbpediaMatch'][0]
                w = self.qwdict.get(q,"")
                if self.dbpwdict.get(dbp, "") != "" and self.dbpwdict.get(dbp, "") != w: 
                    print("Wikidata Uri diff for {} and {}: {} vs {}".format(q,dbp,w,self.dbpwdict.get(dbp, "")))
        ...
        for q in self._qdict:
            if 'http://qudt.org/schema/qudt/dbpediaMatch' in self._qdict[q]:
                dbp = self._qdict[q]['http://qudt.org/schema/qudt/dbpediaMatch'][0]
                w = self.qwdict.get(q,"")
                if self.dbpwdict.get(dbp, "") != "" and self.dbpwdict.get(dbp, "") != w: 
                    print("Wikidata Uri diff for {} and {}: {} vs {}".format(q,dbp,w,self.dbpwdict.get(dbp, "")))
                    if q not in self.qwdict: 
                        ...
                        self.qwdict[q] = self.dbpwdict[dbp]
                self.qdbpdict[q] = dbp
        print("length self.qwdict after: ", len(self.qwdict)) #5 quantities have missing units or dimension in qudt but are defined in wikidata

        self.ucum_u_dict = {} #ucum to qudt unit
        for u in self.udict:
            if 'http://qudt.org/schema/qudt/ucumCode' in self.udict[u]: 
                if self.udict[u]['http://qudt.org/schema/qudt/ucumCode'][0] in self.ucum_u_dict:
                    ucum = self.udict[u]['http://qudt.org/schema/qudt/ucumCode'][0]
                    print("Warning: Conflict for {}: {} vs {}".format(ucum, u, self.ucum_u_dict[ucum]))
                self.ucum_u_dict[self.udict[u]['http://qudt.org/schema/qudt/ucumCode'][0]] = u
        ...
        for wu in self.wudict:
            if 'http://www.wikidata.org/prop/direct/P7825' in self.wudict[wu]:# and 'http://www.wikidata.org/prop/direct/P5061' in self.wudict[wu]:
                #print(self.wudict[wu]['http://www.wikidata.org/prop/direct/P5061'])
                for ucum in self.wudict[wu]['http://www.wikidata.org/prop/direct/P7825']:
    ...
    def set_main_and_invalid_units(self, qId):
        ...
        if 'http://qudt.org/schema/qudt/applicableUnit' in q and not isinstance(q['http://qudt.org/schema/qudt/applicableUnit'], type(None)):
            for u in q['http://qudt.org/schema/qudt/applicableUnit']:
                #print(self.udict[u])
                if 'http://qudt.org/schema/qudt/conversionMultiplier' in self.udict[u]:
                    cf = self.udict[u]['http://qudt.org/schema/qudt/conversionMultiplier'][0]
    ...
    def mergeQudtAndWikidata(self):
        for q in self.qdict:
            if 'http://qudt.org/schema/qudt/applicableUnit' in self.qdict[q] and isinstance(self.qdict[q]['http://qudt.org/schema/qudt/applicableUnit'], type(None)):
                self.qdict[q]['http://qudt.org/schema/qudt/applicableUnit'] = []
                
        remove_list = []
        for q in self.qdict:
            invalid = False
            if not 'http://qudt.org/schema/qudt/hasDimensionVector' in self.qdict[q]:
        ...
        for q in remove_list: del self.qdict[q]
        if 'http://qudt.org/vocab/quantitykind/InformationFlowRate' in self.qdict['http://qudt.org/vocab/quantitykind/DataRate']['http://www.w3.org/2004/02/skos/core#broader']:
            self.qdict['http://qudt.org/vocab/quantitykind/DataRate']['http://www.w3.org/2004/02/skos/core#broader'].remove('http://qudt.org/vocab/quantitykind/InformationFlowRate') #invalid super quantity
        self.qdict['http://qudt.org/vocab/quantitykind/MassFraction']['http://www.w3.org/2004/02/skos/core#broader'] = ['http://qudt.org/vocab/quantitykind/DimensionlessRatio']
        self.qdict['http://qudt.org/vocab/quantitykind/MassFractionOfWater']['http://www.w3.org/2004/02/skos/core#broader'] = ['http://qudt.org/vocab/quantitykind/MassFraction']

        self.udict['http://qudt.org/vocab/unit/UNITLESS']['http://qudt.org/schema/qudt/ucumCode'] = ['[1]']
        self.udict['http://qudt.org/vocab/unit/PERCENT']['http://qudt.org/schema/qudt/conversionMultiplier'] = ['0.01']
        ...
        for q in self.qdict:
            if (len(self.qdict[q]['base_quantity']) > 0): sup = self.qdict[q]['base_quantity'][0]
            else: sup = ""
            if q in self.qwdict:
                w = self.qwdict[q]
                if w in self.wdict:
                    if (len(self.wdict[w]['base_quantity']) > 0): wsup = self.wdict[w]['base_quantity'][0]
                    else: wsup = ""
                    if wsup in self.qwdict_inv:
                        ...
                        if sup != wsup_q:
                            if sup == "": 
                                ...
                                i = 1
                            elif wsup_q in self.qdict and sup in self.qdict[wsup_q]['base_quantity']: 
        ...
        for q in self.qdict:
            invalid = False
            if not 'http://qudt.org/schema/qudt/applicableUnit' in self.qdict[q]: 
        ...
        dim_cluster = []
        for q in self.qdict: 
            if len(self.qdict[q]['base_quantity']) > 0: broader_cluster.append(self.qdict[q]['base_quantity'][0])
            else: broader_cluster.append(q)
            dim_cluster.append(self.qdict[q]['http://qudt.org/schema/qudt/hasDimensionVector'][0])    
                
        self.bres = Counter(broader_cluster)
        self.bres = {k: v for k, v in sorted(self.bres.items(), key=lambda item: item[1], reverse=True)}
        self.dres = Counter(dim_cluster)
        self.dres = {k: v for k, v in sorted(self.dres.items(), key=lambda item: item[1], reverse=True)}
        self.stats = {}
        for dim in self.dres:
            self.stats[dim] = {'total': self.dres[dim], 'broader_cluster_member_count': 0, 'broader_cluster':{}}
        for q in self.bres:
            d = self.qdict[q]['http://qudt.org/schema/qudt/hasDimensionVector'][0]
            self.stats[d]['broader_cluster'][q] = {'member_count': self.bres[q]}
            self.stats[d]['broader_cluster_member_count'] += self.bres[q]
        # print(self.stats)
        
        for q in self.qdict:
            if 'http://qudt.org/vocab/dimensionvector/A0E0L3I0M0H0T0D0' in self.qdict[q]['http://qudt.org/schema/qudt/hasDimensionVector']:
                print(q)
    ...
    def get_wikipage_dict(self, q):
        wt = {'title': self.get_property_wiki_page_name(q), 'content': [{'OslTemplate:Property/Quantity':{}}]}
        wtq = wt['content'][0]['OslTemplate:Property/Quantity']
        wtq['label'] = self.qdict[q]['http://www.w3.org/2000/01/rdf-schema#label'][0] 
        if q in self.qwdict and 'http://www.w3.org/2000/01/rdf-schema#label' in self.wdict[self.qwdict[q]]:
            #wtq['label_de'] = getLangLabel(self.wdict[self.qwdict[q]]['http://www.w3.org/2000/01/rdf-schema#label'], 'de')
            wtq['label'] = self.wdict[self.qwdict[q]]['http://www.w3.org/2000/01/rdf-schema#label']
        ...
        else: wtq['label'] = self.qdict[q]['http://www.w3.org/2000/01/rdf-schema#label'][0] + "@en"         
        wtq['ontology_iri'] = [q]
        if q in self.qwdict: wtq['ontology_iri'].append(self.qwdict[q])
        if q in self.qdbpdict: wtq['ontology_iri'].append(self.qdbpdict[q])
        ...
        if len(self.qdict[q]['base_quantity']) == 0:
            ...
            for cf in self.qdict[q]['units_by_conversion_factor']:
                ...
                valid = False
                first = True
                for index, u in enumerate(self.qdict[q]['units_by_conversion_factor'][cf]):
                    #print(u)
                    unit_uris.append(u)
                    if 'http://qudt.org/schema/qudt/ucumCode' not in self.udict[u]: 
            ...
            wtq['base_quantity'] = self.get_property_wiki_page_name(self.qdict[q]['base_quantity'][0])
            wtq['subquantity_of']=[]
            for sup in self.qdict[q]['subquantity_of']: wtq['subquantity_of'].append(self.get_property_wiki_page_name(sup))
            wtq['display_units'] = []
            wtq['input_units'] = []
            for cf in self.qdict[q]['units_by_conversion_factor']:
                for index, u in enumerate(self.qdict[q]['units_by_conversion_factor'][cf]):
