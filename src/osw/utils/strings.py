...
class RegExPatternExtended(OswBaseModel):
    ...
    def search(self, string: str) -> "SearchResult":
        """Return the search result of the given string and the pattern."""
        return SearchResult(match=re.search(self.pattern, string), pattern=self)
    ...
    def findall(self, string: str) -> Dict[str, List[str]]:
        """Return a list of all match groups of the given string and the pattern."""
        result = {}
        for grp_key in self.group_keys:
            result[grp_key] = [mr.groups[grp_key] for mr in self.finditer(string)]
        return result

    def findall_by_group_key(self, string: str, group_key: str) -> List[str]:
        """Return a list of all match groups of the given string and the pattern by
        group key."""
        if group_key not in self.group_keys:
            raise ValueError(f"Group key '{group_key}' not found in pattern.")
        return self.findall(string)[group_key]
